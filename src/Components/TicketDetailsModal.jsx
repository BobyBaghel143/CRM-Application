...
function TicketDetailsModal({ ticket }) {
  ...
  return (
    <dialog id="ticket_modal" className="modal">
      <div className="modal-box  text-blue-100  ">
        <h3 className="font-bold text-lg">{currentTicket.title} </h3>
        {/* <p className="py-4 " > {ticket.description} </p> */}
        <textarea
          className="bg-gray-700 text-white my-2 rounded-lg w-full resize-none p-2 "
          name="description"
          cols="50"
          rows="7"
        ...
        <h1  className=" text-lg text-white rounded-lg my-2 "  >
          Priority :
        ...
        <h1  className=" text-lg text-white rounded-lg "  >
          Status :
        ...
        <div className="modal-action">
          <button
            className="btn-success  px-4 py-1 rounded-md font-semibold hover:bg-green-300 transition-all ease-in-out duration-300 "
            onClick={handleFormSubmit}> Update ticket
          </button>
        </div>
      </div>
